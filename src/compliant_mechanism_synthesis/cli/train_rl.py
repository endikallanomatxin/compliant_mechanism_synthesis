...
from compliant_mechanism_synthesis.training import RLTrainingConfig, run_rl_training
from compliant_mechanism_synthesis.utils import timestamped_run_dir


def _build_parser() -> argparse.ArgumentParser:
    defaults = RLTrainingConfig(dataset_path="")
    parser = argparse.ArgumentParser(
        prog="cms-train-rl",
        description=(
            "Fine-tune the refiner online against final mechanics objectives using "
            "rollouts from structured noise."
        ),
    )
...
def train_rl_main(argv: list[str] | None = None) -> None:
